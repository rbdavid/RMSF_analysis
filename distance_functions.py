...
def RMSD_calc(analysis_universe, analysis_universe_all_selection, analysis_universe_alignment_selection, reference_alignment_positions, selection_list, nAtoms_list, reference_positions_list, trajectory_list, data_output_filename, wrapping_boolean = False, wrapping_selection = None):
        """
        """

        if wrapping_boolean:
        	wrapping_atomgroup = analysis_universe.select_atoms(wrapping_selection)
        	nResidues_range = range(wrapping_atomgroup.n_residues)

        nSel_range = range(len(selection_list))
	print 'Beginning trajectory analysis'
        with open(data_output_filename,'w') as f:
                for i in trajectory_list:
                        ...
                        analysis_universe.load_new(i)
                        for ts in analysis_universe.trajectory:
			        analysis_universe_all_selection.translate(-analysis_universe_alignment_selection.center_of_mass())  # Align to reference (moves COM of alignment to origin)
                                if wrapping_boolean:
                                        dimensions = analysis_universe.dimensions[:3]
                                        for j in nResidues_range:
                                                COM = wrapping_atomgroup.residues[j].center_of_mass()
                                                t = wrapping(COM,dimensions)
                                                wrapping_atomgroup.residues[j].atoms.translate(t)
			        
                                R, rmsd = rotation_matrix(analysis_universe_alignment_selection.positions, reference_alignment_positions)  # Calculate the rotational matrix to align u to the ref
			        analysis_universe_all_selection.rotate(R)     # Apply rotation matrix to atoms within u
			        for j in nSel_range:      # loop through selections and compute RMSD
			        	rmsd = RMSD(selection_list[j].positions,reference_positions_list[j],nAtoms_list[j])
			        	f.write('%10.6f   ' %(rmsd))
			        f.write('\n')
	print 'Done with trajectory analysis'
        return

def RMSF_calc(analysis_universe, analysis_universe_all_selection, analysis_universe_alignment_selection, reference_alignment_positions, selection_list, nAtoms_list, reference_positions_list, trajectory_list, data_output_filename, wrapping_boolean = False, wrapping_selection = None):
        """
        """
        ...
        nSteps = 0
        dist2 = np.zeros(len(selection_list))
        nSel_range = range(len(selection_list))
	print 'Beginning trajectory analysis'
        for i in trajectory_list:
                print 'Loading trajectory', i
                analysis_universe.load_new(i)
                nSteps += len(analysis_universe.trajectory)
                for ts in analysis_universe.trajectory:
		        analysis_universe_all_selection.translate(-analysis_universe_alignment_selection.center_of_mass())  # Align to reference (moves COM of alignment to origin)
                        if wrapping_boolean:
                                dimensions = analysis_universe.dimensions[:3]
                        ...
                        R, d = rotation_matrix(analysis_universe_alignment_selection.positions, reference_alignment_positions)  # Calculate the rotational matrix to align u to the ref
		        analysis_universe_all_selection.rotate(R)     # Apply rotation matrix to atoms within u
		        for j in nSel_range:      # loop through selections and compute RMSD
		        	dist2[j] += MSD(selection_list[j].positions,reference_positions_list[j],nAtoms_list[j])
